...
import { DEMO_RESTAURANT_ID } from "@/lib/mock-data"
...
export default function MenuPage() {
  ...
  const handleSaveProduct = async (productFormData: FoodItem & { category_id: string }) => {
    ...
    if (!user) {
      toast({ title: "Hata", description: "Kullanıcı bilgisi eksik.", variant: "destructive" })
      return
    }

    // Get restaurant_id from user or use demo restaurant_id in demo mode
    const restaurantId = user.restaurant_id || (isDemoMode() ? DEMO_RESTAURANT_ID : null)
    if (!restaurantId) {
      toast({ title: "Hata", description: "Restoran bilgisi eksik.", variant: "destructive" })
      return
    }
    ...
    try {
      ...
      const payload = {
        ...
        restaurant_id: restaurantId, // Add restaurant_id from auth user or demo
      }
