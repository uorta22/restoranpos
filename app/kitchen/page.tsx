...
export default function KitchenPage() {
  ...
  const { orders, updateOrderStatus, updateDeliveryStatus, updatePaymentStatus } = useOrderContext()
  const { getAvailableCouriers, assignOrderToCourier } = useCourierContext()
  ...
  const handleStatusChange = async (
    ...
    try {
      // Kuryeye verilecek siparişler için müsait kurye kontrolü ve otomatik atama
      if (status === OrderStatus.OUT_FOR_DELIVERY) {
        const availableCouriers = getAvailableCouriers()
        if (availableCouriers.length === 0) {
          toast({
            title: "Müsait Kurye Yok",
            description: "Sipariş kuryeye verilebilmesi için en az bir müsait kurye olmalıdır.",
            variant: "destructive",
          })
          return
        }

        // İlk müsait kuryeyi otomatik olarak ata
        const selectedCourier = availableCouriers[0]
        assignOrderToCourier(selectedCourier.id, orderId)
        updateDeliveryStatus(orderId, "Yolda")

        toast({
          title: "Kurye Atandı",
          description: `Sipariş ${selectedCourier.name} kuryesine atandı ve yola çıktı.`,
        })
      }

      await updateOrderStatus(orderId, status)

      // Eğer sipariş tamamlandıysa ödeme durumunu da güncelle
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {order.status === OrderStatus.PREPARING && (
                          <Button
                            onClick={() => {
                              // Gel-Al siparişleri için direkt tamamlandı statüsüne geç
                              if (order.orderType === OrderType.TAKEAWAY) {
                                handleStatusChange(order.id, OrderStatus.COMPLETED)
                              } else {
                                handleStatusChange(order.id, OrderStatus.READY_FOR_SERVICE)
                              }
                            }}
                            className="w-full bg-green-600 hover:bg-green-700"
                          >
                            {order.orderType === OrderType.TAKEAWAY ? "Teslim Et" : "Hazır"}
                          </Button>
                        )}
                        {order.status === OrderStatus.READY_FOR_SERVICE && (
                          <>
