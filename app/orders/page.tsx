      ...
      case "Beklemede":
        return (
          <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">
            <Clock className="mr-1 h-3 w-3" /> Beklemede
          </Badge>
        )
      case "Hazırlanıyor":
        return (
        ...
        <div className="flex-1 overflow-auto p-4">
          ...
          <Tabs defaultValue="all">
            ...
            <TabsContent value="preparing">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredOrders.filter((order) => order.status === "Hazırlanıyor").length === 0 ? (
                  <div className="col-span-full text-center py-12">
                    <p className="text-gray-500">Hazırlanan sipariş bulunmuyor.</p>
                  </div>
                ) : (
                  filteredOrders
                    .filter((order) => order.status === "Hazırlanıyor")
                    .map((order) => (
                      <Card key={order.id}>
                        <CardHeader className="pb-2">
                          <div className="flex justify-between items-start">
            ...
            <TabsContent value="ready">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredOrders.filter((order) => order.status === "Hazır").length === 0 ? (
                  <div className="col-span-full text-center py-12">
                    <p className="text-gray-500">Hazır sipariş bulunmuyor.</p>
                  </div>
                ) : (
                  filteredOrders
                    .filter((order) => order.status === "Hazır")
                    .map((order) => (
                      <Card key={order.id}>
                        <CardHeader className="pb-2">
                          <div className="flex justify-between items-start">
          ...
          {currentOrder && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm">{getPaymentStatusBadge(currentOrder.paymentStatus)}</p>
